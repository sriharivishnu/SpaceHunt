...
namespace ProBuilder2.Examples
{
	public class CameraControls : MonoBehaviour
	{
		...
		void LateUpdate()
		{
			...
			{
				eulerRotation.y += Time.smoothDeltaTime * idleRotation * dir.x;
				eulerRotation.x += Time.smoothDeltaTime * Mathf.PerlinNoise(Time.time, 0f) * idleRotation * dir.y;
			}
			...
			if( Input.GetAxis(INPUT_MOUSE_SCROLLWHEEL) != 0f )
			{
				...
				distance -= delta * (distance/MAX_CAM_DISTANCE) * (zoomSpeed * 1000) * Time.smoothDeltaTime;
